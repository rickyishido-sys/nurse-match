-- HANAKAI usage fee platform settings (admin-editable in future; V1 seed only).

create table if not exists public.hanakai_platform_settings (
  ...
  updated_by_member_id uuid references public.hanakai_members(id) on delete set null
);

insert into public.hanakai_platform_settings (key, value_int, description)
values ('usage_fee_jpy', 500, 'HANAKAI利用料（税込・JPY）。管理画面から変更予定')
on conflict (key) do nothing;

comment on table public.hanakai_platform_settings is
  'Platform-wide HANAKAI settings. V1: seed only; future admin UI will update rows here.';

-- Relax fixed-500 check so fee changes / campaigns do not require DDL.
alter table public.hanakai_participation_payments
  drop constraint if exists hanakai_participation_payments_amount_check;

alter table public.hanakai_participation_payments
  add constraint hanakai_participation_payments_amount_check
  check (amount >= 0 and amount <= 100000);

-- RPC: read usage fee from settings (fallback 500)
create or replace function public.hanakai_get_usage_fee_jpy()
returns int
language sql
stable
security definer
...
as $$
  select coalesce(
    (select value_int from public.hanakai_platform_settings where key = 'usage_fee_jpy'),
    500
  );
$$;

revoke all on function public.hanakai_get_usage_fee_jpy() from public;
grant execute on function public.hanakai_get_usage_fee_jpy() to service_role;

-- Patch select-participants RPC to use settings-backed fee
...
declare
  ...
  v_usage_fee int;
begin
  v_usage_fee := public.hanakai_get_usage_fee_jpy();
