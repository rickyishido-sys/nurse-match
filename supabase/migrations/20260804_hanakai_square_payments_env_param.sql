-- Parameterize the participation-payment environment (sandbox|production).
-- Supersedes the 4-arg hanakai_select_participants_for_payment from
-- 20260802_hanakai_square_payments.sql, which hardcoded environment='sandbox'
-- and therefore made real Production charges impossible (payment rows were
-- always 'sandbox' while cards are saved under getSquareEnvironment()).
--
-- Design notes:
--   * The legacy 4-arg overload is DROPPED so PostgREST resolves the RPC
--     unambiguously to the single 5-arg definition. A 4-key/4-positional call
--     still works because p_environment has a DEFAULT ('sandbox'), so Preview
--     behavior is preserved even if a caller has not been updated.
--   * DOES NOT modify 20260802_hanakai_square_payments.sql.
--   * Only service_role may EXECUTE. public/anon/authenticated are REVOKEd.

-- 1. Remove the ambiguous legacy 4-arg overload.
drop function if exists public.hanakai_select_participants_for_payment(uuid, uuid[], uuid, timestamptz);

-- 2. Recreate as a 5-arg function with an explicit, validated environment.
...
declare
  ...
  v_payment_id uuid;
begin
  -- Reject anything other than the two supported environments.
  if p_environment is null or p_environment not in ('sandbox', 'production') then
    raise exception 'HANAKAI_INVALID_ENVIRONMENT: %', coalesce(p_environment, 'null')
      using errcode = '22023';
  end if;
  ...
  loop
    ...
    insert into public.hanakai_participation_payments (
      ...
      500, 'JPY', 'pending',
      v_idempotency, 1,
      v_payment_deadline, p_environment
    )
...
-- 3. Lock down EXECUTE to service_role only (defence in depth).
revoke all on function public.hanakai_select_participants_for_payment(uuid, uuid[], uuid, timestamptz, text) from public;
revoke all on function public.hanakai_select_participants_for_payment(uuid, uuid[], uuid, timestamptz, text) from anon;
revoke all on function public.hanakai_select_participants_for_payment(uuid, uuid[], uuid, timestamptz, text) from authenticated;
grant execute on function public.hanakai_select_participants_for_payment(uuid, uuid[], uuid, timestamptz, text) to service_role;
